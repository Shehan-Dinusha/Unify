...
router.get("/packages/:id", BoostController.getPackageById);
...
router.post("/confirm-payment", protect, BoostController.confirmBoostPayment);

// ── Purchase (User) — Direct Purchase (no Stripe) ───────────────────────────

//Purchase a boost package (direct, no Stripe)
router.post("/purchase", protect, BoostController.purchaseBoost);

//Get user's active boosts where expiryDate > NOW
router.get("/my-boosts", protect, BoostController.getMyBoosts);

// ── Admin ────────────────────────────────────────────────────────────────────

//Admin configuration changes logs
router.get("/admin/logs", protect, authorize("Admin"), BoostController.getLogs);
