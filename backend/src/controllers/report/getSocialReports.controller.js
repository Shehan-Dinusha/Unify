  ...
  if (notesField) {
    ...
    noteLines.forEach((line, idx) => {
      ...
      if (line.includes("Dismiss Reason:")) {
        ...
        line.includes("Action Taken: Suspended User") ||
        line.includes("Action: Suspended User")
      ) {
        ...
        line.includes("Admin viewed report") ||
        line.includes("viewed report")
      ) {
    ...
    const formatted = await Promise.all(
      reports.map(async (r) => ({
        ...
        reportedUser: {
          ...
          handle: r.student?.name
            ? `@${r.student.name.toLowerCase().replace(/\s+/g, "_")}`
            : `@student_${r.studentId}`,
...
export const getSocialReportById = async (req, res, next) => {
  try {
    ...
    if (r.status === "Pending Review" || r.status === "Pending") {
      ...
      if (isStudentReport)
        r.adminNotes = (r.adminNotes ? r.adminNotes + "\n" : "") + viewNote;
    ...
    if (isStudentReport) {
      ...
      const isNumericId =
        !isNaN(entityId) && /^\d+$/.test(String(r.reportedEntityId));
      ...
      if (r.reportType === "post" && isNumericId) {
        ...
        if (!post) {
          ...
          if (item) {
            offender = {
              ...
              name: item.seller?.name || "Deleted Business",
              avatar: await resolveAvatarUrl(
                item.seller?.avatar,
          ...
          reportedContent = {
            id: String(post.id),
            text: `${post.description || post.title || "Post Content"}${post.deletedAt ? " [DELETED]" : ""}`,
            author: offender.name,
            avatar: offender.avatar,
            date: moment(post.createdAt).format("MMM DD, YYYY"),
            hasImage: !!post.images,
            imageLabel: post.deletedAt ? "DELETED CONTENT" : "POST IMAGE",
          };
        }
        ...
        if (comment) {
          ...
          reportedContent = {
            ...
            text: `${comment.content}${comment.deletedAt ? " [DELETED]" : ""}`,
            author: offender.name,
            avatar: offender.avatar,
            date: moment(comment.createdAt).format("MMM DD, YYYY"),
          };
        }
        ...
        if (user) {
          ...
          reportedContent = {
            id: String(user.id),
            text: `User Profile: ${user.name}${user.deletedAt ? " [DELETED]" : ""}`,
            author: user.name,
            avatar: offender.avatar,
            date: moment(user.createdAt).format("MMM DD, YYYY"),
          };
        }
      }
    }
    if (!isStudentReport) {
      ...
      if (r.post)
        reportedContent = {
          id: String(r.post.id),
          text: r.post.description || r.post.title,
          author: offender.name,
          avatar: offender.avatar,
          date: moment(r.post.createdAt).format("MMM DD, YYYY"),
          hasImage: !!r.post.images,
        };
      else if (r.comment)
        reportedContent = {
          id: `comment_${r.comment.id}`,
          text: r.comment.content,
          author: offender.name,
          avatar: offender.avatar,
          date: moment(r.comment.createdAt).format("MMM DD, YYYY"),
        };
      else reportedContent.text = r.description || "No description";
    }

    offender.handle = offender.name
      ? `@${offender.name.toLowerCase().replace(/\s+/g, "_")}`
      : "@unknown";
    ...
    return sendResponse(res, 200, true, "Detail retrieved", {
      ...
      reportedBy: {
        ...
        handle: isStudentReport
          ? r.student?.name
            ? `@${r.student.name.toLowerCase().replace(/\s+/g, "_")}`
            : "@student"
          : r.reporter?.name
            ? `@${r.reporter.name.toLowerCase().replace(/\s+/g, "_")}`
            : "@user",
        avatar: isStudentReport
