...
describe("createItem Controller", () => {
  it("returns 201 and creates item successfully (no files)", async () => {
    ...
    const { createItem } =
      await import("../../../../src/controllers/lostAndFound/createItem.controller.js");
  ...
  it("extracts S3 keys from uploaded files and stores them", async () => {
    ...
    assert.deepEqual(capturedData.images, [
      "lost-and-found/abc123.jpg",
      "lost-and-found/def456.jpg",
    ]);
  });
...
describe("getItems Controller", () => {
  ...
  it("returns 200 with a formatted item list", async () => {
    ...
    const { getItems } =
      await import("../../../../src/controllers/lostAndFound/getItems.controller.js");
...
describe("getItemById Controller", () => {
  it("returns 404 when item does not exist", async () => {
    ...
    const { getItemById } =
      await import("../../../../src/controllers/lostAndFound/getItemById.controller.js");
...
describe("deleteItem Controller", () => {
  it("returns 401 when user is not authenticated", async () => {
    const { deleteItem } =
      await import("../../../../src/controllers/lostAndFound/deleteItem.controller.js");

    const res = mockRes();
    await runController(
      deleteItem,
      { user: null, params: { id: "1" } },
      res,
      mockNext(),
    );
  ...
  it("deletes S3 images before destroying the DB record", async () => {
    ...
    mock.method(LostAndFound, "findByPk", async () => item);
    mock.method(s3Service, "deleteFile", async (key) => {
    ...
    assert.deepEqual(
      deletedKeys.sort(),
      ["lost-and-found/img1.jpg", "lost-and-found/img2.jpg"].sort(),
    );
    assert.equal(item.destroy.mock.calls.length, 1);
  });
...
describe("claimItem Controller", () => {
  it("returns 404 when the item does not exist", async () => {
    ...
    const { claimItem } =
      await import("../../../../src/controllers/lostAndFound/claimItem.controller.js");
    ...
    await runController(
      claimItem,
      {
        user: { id: 2 },
        params: { id: "999" },
        body: { contactNumber: "0771234567", description: "Mine" },
      },
      res,
      mockNext(),
    );
  ...
  it("returns 400 when an existing claim already exists for this user", async () => {
    ...
    mock.method(ClaimRequest, "findOne", async () => ({
      id: 10,
      itemId: 1,
      claimantId: 2,
    }));
    ...
    await runController(
      claimItem,
      {
        ...
        body: {
          ...
          description: "I think this is mine",
        },
      },
      res,
      mockNext(),
  ...
  it("forwards DB errors to next()", async () => {
    ...
    await runController(
      claimItem,
      {
        user: { id: 2 },
        params: { id: "1" },
        body: { contactNumber: "077", description: "Mine" },
      },
      mockRes(),
      next,
    );
...
describe("editItem Controller", () => {
  it("returns 404 when item does not exist", async () => {
    ...
    const { editItem } =
      await import("../../../../src/controllers/lostAndFound/editItem.controller.js");
