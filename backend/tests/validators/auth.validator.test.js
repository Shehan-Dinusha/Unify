...
describe("registerValidator", () => {
  ...
  it("rejects weak password (no uppercase)", async () => {
    ...
    assert.match(err, /uppercase, lowercase, number, and special character/i);
  });
  ...
  it("rejects short password", async () => {
    ...
    assert.match(err, /at least 8 characters long/i);
  });
  ...
  it("rejects missing role", async () => {
    ...
    assert.match(err, /Role is required/i);
  });
  ...
  it("rejects invalid role", async () => {
    ...
    assert.match(err, /Invalid role specified/i);
  });
  ...
  it("rejects Business registration without email and phone", async () => {
    ...
    assert.match(
      err,
      /Business accounts require either an email or a phone number/i,
    );
  });
  ...
  it("rejects invalid phone number format", async () => {
    ...
    assert.match(err, /Invalid phone number format/i);
  });
...
describe("verifyOTPValidator", () => {
  it("accepts valid OTP with email", async () => {
    assert.equal(
      await getError(verifyOTPValidator, {
        email: "john@uom.lk",
        otp: "123456",
      }),
      null,
  ...
  it("rejects OTP not 6 digits", async () => {
    const err = await getError(verifyOTPValidator, {
      email: "john@uom.lk",
      otp: "12345",
    });
    assert.match(err, /OTP must be 6 digits/i);
  });

  it("rejects missing OTP", async () => {
    const err = await getError(verifyOTPValidator, { email: "john@uom.lk" });
    assert.match(err, /OTP code is required/i);
  });

  it("rejects missing email and phone", async () => {
    const err = await getError(verifyOTPValidator, { otp: "123456" });
    assert.match(err, /Either email or phone number is required/i);
  });
});
...
describe("resendOTPValidator", () => {
  it("accepts email", async () => {
    assert.equal(
      await getError(resendOTPValidator, { email: "john@uom.lk" }),
      null,
    );
  });
  ...
  it("accepts phone", async () => {
    assert.equal(
      await getError(resendOTPValidator, { phone: "+94771234567" }),
      null,
    );
  });
...
describe("forgotPasswordValidator", () => {
  it("accepts email", async () => {
    assert.equal(
      await getError(forgotPasswordValidator, { email: "john@uom.lk" }),
      null,
    );
  });
  ...
  it("rejects empty body", async () => {
    ...
    assert.match(err, /Either a valid email or phone number is required/i);
  });
});
...
describe("verifyResetOTPValidator", () => {
  it("accepts valid OTP with email", async () => {
    assert.equal(
      await getError(verifyResetOTPValidator, {
        email: "john@uom.lk",
        otp: "654321",
      }),
      null,
  ...
  it("rejects missing OTP", async () => {
    const err = await getError(verifyResetOTPValidator, {
      email: "john@uom.lk",
    });
    assert.match(err, /OTP code is required/i);
  });
...
describe("resetPasswordValidator", () => {
  it("accepts valid reset data", async () => {
    assert.equal(
      await getError(resetPasswordValidator, {
        email: "john@uom.lk",
        otp: "123456",
        password: "NewPass123!",
      }),
  ...
  it("rejects short password", async () => {
    ...
    assert.match(err, /Password must be at least 6 characters/i);
  });
});
