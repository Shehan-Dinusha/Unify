...
describe("studentDirectoryValidator", () => {
  ...
  it("accepts valid status filter: Active", async () => {
    assert.equal(
      await getErrorWithQuery(studentDirectoryValidator, { status: "Active" }),
      null,
    );
  });
  ...
  it("accepts valid status filter: Suspended", async () => {
    assert.equal(
      await getErrorWithQuery(studentDirectoryValidator, {
        status: "Suspended",
      }),
      null,
    );
  ...
  it("accepts status filter: all", async () => {
    assert.equal(
      await getErrorWithQuery(studentDirectoryValidator, { status: "all" }),
      null,
    );
  });
  ...
  it("rejects invalid status", async () => {
    const err = await getErrorWithQuery(studentDirectoryValidator, {
      status: "Banned",
    });
  ...
  it("accepts search query", async () => {
    assert.equal(
      await getErrorWithQuery(studentDirectoryValidator, { search: "john" }),
      null,
    );
  });
...
describe("businessDirectoryValidator", () => {
  ...
  it("accepts valid status: Active", async () => {
    assert.equal(
      await getErrorWithQuery(businessDirectoryValidator, { status: "Active" }),
      null,
    );
  });
  ...
  it("accepts status: all", async () => {
    assert.equal(
      await getErrorWithQuery(businessDirectoryValidator, { status: "all" }),
      null,
    );
  });

  it("rejects invalid status", async () => {
    const err = await getErrorWithQuery(businessDirectoryValidator, {
      status: "Deleted",
    });
    assert.match(err, /Invalid status/);
  });
  ...
  it("accepts category filter", async () => {
    assert.equal(
      await getErrorWithQuery(businessDirectoryValidator, {
        category: "Food & Cafe",
      }),
      null,
    );
  ...
  it("accepts search query", async () => {
    assert.equal(
      await getErrorWithQuery(businessDirectoryValidator, { search: "pizza" }),
      null,
    );
  });
...
describe("updateStatusValidator", () => {
  ...
  it("rejects missing status", async () => {
    ...
    assert.match(err, /Status is required/i);
  });
  ...
  it("rejects non-integer id param", async () => {
    ...
    assert.match(err, /User ID must be an integer/i);
  });
  ...
  it("rejects non-boolean sendEmail", async () => {
    ...
    assert.match(err, /sendEmail must be a boolean/i);
  });
...
describe("addNoteValidator", () => {
  ...
  it("rejects missing text", async () => {
    ...
    assert.match(err, /Note text is required/i);
  });
  ...
  it("accepts note with special characters", async () => {
    const err = await getError(
      addNoteValidator,
      { text: 'Note with <html> & "special" chars ✅' },
      { id: "1" },
    );
    assert.equal(err, null);
  });
...
describe("sendWarningValidator", () => {
  ...
  it("rejects missing message", async () => {
    ...
    assert.match(err, /Warning message is required/i);
  });
  ...
  it("rejects missing category", async () => {
    const err = await getError(
      sendWarningValidator,
      {
        message: "This is a formal warning.",
        severity: "Level 1 - Formal Caution",
      },
      { id: "1" },
    );
    assert.match(err, /Violation category is required/i);
  });
  ...
  it("rejects missing severity", async () => {
    ...
    assert.match(err, /Severity level is required/i);
  });
