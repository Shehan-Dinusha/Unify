...
import {
  ...
  ReviewStarAvatar,
  ReviewReplyAvatar,
  ReviewFeedbackAvatar,
  ReviewFeedbackNotHelpfulAvatar,
  FollowerAvatar,
  OrderIcon,
  ClaimAvatar,
  WarningIcon,
...
const NotificationCard = ({ notification, onMarkRead, onNavigate }) => {
  ...
  const renderIcon = () => {
    switch (type) {
      ...
      default:
        ...
        if (notification.referenceType === "Suspension")
          return <SuspensionIcon />;
        if (
          ["Reactivation", "Report", "ContentRemoval", "ForceLogout"].includes(
        ...
        if (notification.referenceType === "Review") {
          switch (notification.reviewAction) {
            case "reply":
              return <ReviewReplyAvatar avatar={avatar} />;
            case "feedback":
              return <ReviewFeedbackAvatar avatar={avatar} />;
            case "new":
            default:
              return <ReviewStarAvatar avatar={avatar} />;
          }
        }
  ...
  const renderTitle = () => {
    ...
    if (
      ["Suspension", "ForceLogout", "ContentRemoval"].includes(
        notification.referenceType,
      )
    ) {
  ...
  return (
    ...
    >
      <div
        className="flex items-start gap-4 p-5 sm:p-6 w-full h-full relative"
        onClick={handleClick}
      >
        {renderIcon()}
        <div className="flex-1 flex flex-col gap-1.5 pt-0.5">
          ...
          <p
            className={`text-body-small ${type === "reply" ? "text-text-secondary italic" : "text-text-secondary"}`}
          >
            {content}
          </p>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0 h-full">
          {isUnread && (
            <div className="w-2.5 h-2.5 rounded-full bg-primary-blue mt-2" />
          )}
          {image && (
