...
const orderService = {
  ...
  bulkUpdateBookingStatus: async (bookingIds, status) => {
    try {
      const response = await api.patch(`/bookings/bulk-status`, {
        bookingIds,
        status,
      });
      return response.data;
  ...
  getClubOrderTrends: async (userId, days = 30) => {
    try {
      const response = await api.get(
        `/orders/analytics/trends/${userId}?days=${days}`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
  ...
  getClubTopProducts: async (userId) => {
    try {
      const response = await api.get(
        `/orders/analytics/top-products/${userId}`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
  ...
  getClubBuyerDemographics: async (userId) => {
    try {
      const response = await api.get(
        `/orders/analytics/demographics/${userId}`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
  ...
  togglePostVisibility: async (type, postId) => {
    try {
      const response = await api.patch(
        `/orders/posts/${type}/${postId}/visibility`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
