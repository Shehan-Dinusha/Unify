...
export const VerificationConfirmationModal = ({ isOpen, onClose, onConfirm }) => {
  if (!isOpen) return null;

  return (
    <ModalBackdrop>
      <Card variant="card" padding="p-0" className="w-full max-w-[440px] overflow-hidden outline outline-1 outline-offset-[-1px] outline-white/10 shadow-2xl">
        <div className="p-8 pb-6 flex flex-col items-center text-center">
          ...
          {/* Text */}
          <h2 className="text-xl font-bold text-white mb-3">Confirm Verification</h2>
          <p className="text-text-secondary text-sm leading-relaxed mb-4">
            Are you sure you want to verify this request? This action will grant the requested permissions and notify the user.
          </p>
        </div>
...
export const VerificationSuccessModal = ({ isOpen, onClose, clubName = "Robotics Club" }) => {
  if (!isOpen) return null;

  return (
    <ModalBackdrop>
      <Card variant="card" className="w-full max-w-[440px] !p-0 overflow-hidden outline outline-1 outline-offset-[-1px] outline-white/10 shadow-2xl">
        <div className="p-8 pb-6 flex flex-col items-center text-center">
          {/* Icon */}
          ...
          {/* Text */}
          <h2 className="text-xl font-bold text-white mb-3">Successfully Verified!</h2>
          <div className="text-text-secondary text-sm leading-relaxed mb-4">
             The <span className="text-white font-semibold">{clubName}</span> has been officially verified and the account owner has been notified.
          </div>
        </div>
...
export const VerificationRejectionModal = ({ isOpen, onClose, onConfirm, clubName = "Robotics Club", requestType = "Club" }) => {
    ...
    const recipientRole = requestType === 'Club' ? 'club representative' : 'batch representative';

    return (
        <ModalBackdrop>
            <Card variant="card" padding="p-0" className="w-full max-w-[440px] h-[500px] overflow-hidden outline outline-1 outline-offset-[-1px] outline-white/10 shadow-2xl">
                <div className="flex flex-col h-full w-full">
                    ...
                    <div className="px-6 py-4 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-body-large-bold text-text-primary leading-tight">Reject Verification</h3>
                                <p className="text-body-small-bold text-text-secondary">Request from: {clubName}</p>
                            </div>
                        </div>
                    ...
                    <div className="p-6 pt-0 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                        {/* Reason Select */}
                        <div>
                             <h4 className="text-body-small-bold text-text-soft mb-3">Select Standard Rejection Reason</h4>
                                ...
                                {reasons.map((reason) => (
                                    ...
                                    >
                                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ${
                                            selectedReason === reason ? 'border-primary-blue bg-primary-blue' : 'border-text-secondary bg-transparent'
                                        }`}>
                                            {selectedReason === reason && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                                        </div>
                                        <span className={`text-body-small ${selectedReason === reason ? 'text-text-primary' : 'text-text-secondary'}`}>
                                            {reason}
                                        </span>
                                    </div>
                                ))}
                        ...
                        {selectedReason === "Other (Specify below)" && (
                            <div className="animate-in slide-in-from-top-2 duration-300 fade-in">
                                <h4 className="text-body-small-bold text-text-soft mb-2">Custom Message to User</h4>
...
export const VerificationRejectedSuccessModal = ({ isOpen, onClose, clubName = "Robotics Club", reason = "Incomplete Documents" }) => {
    if (!isOpen) return null;
                    ...
                    <div className="relative mb-6">
                        <div className="w-20 h-20 bg-red-900/30 rounded-full flex items-center justify-center ring-4 ring-state-error/10">
                            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM17 13H7V11H17V13Z" fill="#F87171"/>
                            </svg>
                        </div>
                    </div>

                    {/* Text */}
                    <h2 className="text-heading-small text-text-primary mb-2">Verification Rejected</h2>
                    <div className="text-body-small text-text-secondary leading-relaxed mb-6">
                        The request from <span className="text-text-primary font-bold">{clubName}</span> has been declined. 
                        <br />
                        The user has been notified with the reason:
                        <br />
                        <span className="text-primary-blue font-bold block mt-1">{reason}</span>
                    </div>
