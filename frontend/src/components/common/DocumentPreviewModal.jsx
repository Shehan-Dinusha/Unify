...
const DocumentPreviewModal = ({ isOpen, onClose, document }) => {
  ...
  const isPdf =
    document.type === "pdf" || document.name?.toLowerCase().endsWith(".pdf");
  const isImage =
    ["jpg", "jpeg", "png", "gif"].includes(document.type) ||
    /\.(jpg|jpeg|png|gif)$/i.test(document.name);
          ...
          <div className="flex items-center gap-3">
            ...
            {isImage && (
              <div className="flex items-center bg-gray-900 rounded-lg border border-white/10 p-1 mr-2">
                ...
                <span className="text-xs text-center min-w-[3rem] text-text-secondary font-medium">
                  {zoomLevel}%
                </span>
                <button
                  onClick={() =>
