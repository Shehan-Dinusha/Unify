...
const SemesterVisibilityModal = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-dark-1/80 backdrop-blur-md transition-all duration-300 px-4">
      <Card
        variant="card"
        className="w-full max-w-[510px] !p-0 flex flex-col relative overflow-visible outline outline-1 outline-offset-[-1px] outline-white/20 shadow-[0px_8px_32px_0px_rgba(31,38,135,0.37)] animate-in fade-in zoom-in duration-200 bg-white/10"
      >
        <div className="px-6 pt-6 pb-4 relative flex flex-col items-start gap-4 w-full">
          <div className="flex justify-start items-start w-full">
            <div className="w-10 h-10 bg-blue-900/30 rounded-full flex justify-center items-center shrink-0">
              <Eye size={20} className="text-blue-500" />
            </div>
            <div className="flex-1 pl-4 flex flex-col gap-2 w-full">
              <h2 className="text-white text-xl font-bold font-inter leading-5">
              ...
              <div className="w-full pt-4 pb-3 flex flex-col gap-3">
                {batches.length > 0 ? (
                  batches.map((batch) => (
                    <div
                      key={batch.id}
                      className="p-3 bg-gray-800 rounded-lg outline outline-1 outline-offset-[-1px] outline-gray-700 flex justify-between items-center text-left"
                    >
              ...
              <div
                className="w-full relative border-t border-primary-blue/20 pt-4 flex gap-2 items-start cursor-pointer group"
        ...
        {/* Footer */}
        <div className="w-full px-6 py-3 border-t border-primary-blue/20 flex justify-end items-center gap-3 rounded-b-3xl">
          <Button
