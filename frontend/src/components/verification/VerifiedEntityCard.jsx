...
const VerifiedEntityCard = ({ entity, onRemoveVerification }) => {
  return (
    <Card variant="container" className="h-full relative overflow-hidden group hover:bg-white/5 transition-colors">
      <div className="flex flex-col gap-lg h-full"> 
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-sm items-center">
            ...
            />
            <div>
              <h3 className="text-body-medium-bold text-text-primary">{entity.name}</h3>
              <div className="flex items-center gap-sm mt-1">
                 <span className={`px-sm py-xs rounded text-body-extra-small-bold font-inter ${
                    entity.type === 'Club' ? 'bg-indigo-900/30 text-indigo-300' : 'bg-purple-900/30 text-purple-300'
                 }`}>
                    {entity.type}
                 </span>
                 <span className="text-text-secondary text-body-extra-small font-normal">• {entity.verifiedDate}</span>
              </div>
            </div>
          </div>
          <div className="p-xs bg-blue-500/10 rounded flex items-center justify-center">
            <img src="/icon_verified_badge.svg" alt="Verified" className="w-3 h-3" />
          </div>
        </div>
        ...
        <div className="p-sm bg-dark-4 rounded-lg border border-white/10 flex flex-col justify-center gap-1">
             <div className="flex justify-between items-center">
                 <p className="text-body-small-bold text-text-secondary">Verified Since</p>
                 <p className="text-body-small text-text-primary">{entity.verifiedDate}</p>
             </div>
             ...
             {entity.type === 'Batch Rep' && (
                <div className="flex justify-between items-center mt-1 pt-1 border-t border-white/5">
                   <p className="text-body-small-bold text-text-secondary">{entity.degree}</p>
                   <p className="text-body-small text-text-primary">{entity.batch}</p>
                </div>
             )}
        </div>
